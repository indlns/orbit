...
class ArchiveReminderCell: UITableViewCell {
    ...
    public func configure(model: Reminders) {
        ...
        dateLabel.text = "Завершено: " + DateFormatHelper.getString(from: model.endDate ?? .now) + " в \(DateFormatHelper.getTimeString(from: model.endDate ?? .now))"
    }
